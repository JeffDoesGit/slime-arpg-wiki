...
Your gems sit in the bag on the [inventory screen](inventory-shell.md) as their stones, each with its tier numeral in the corner of the cell and a frame in its rarity's colour (since E-3.8: light grey for a one-line gem, blue for two lines, gold for three; a gem granted by a dev command with no rarity has a dark grey frame); hover for the stone's name, the rarity and the lines ("Pyrope Garnet IV (R3)"), and since E-3.8 each line reads as it acts, "+14 Health" for points, "+12%" for an increased line, "8% more" for a more line, followed by the line's tier and the band it could have rolled in, "T2 (+11 to +18)", so you can tell a high roll from a low one. A slotted gem's frame is its rarity too. A slotted gem and a gem being dragged show the same stone. A gem whose colour and tier have no stone shows the plain colour icon instead. Drag one onto a slot of its colour to equip it (an iridescent gem, a fifth colour with no lines decided yet, goes only in the two iridescent slots, and the other four colours never do, since E-3.6); drag it back to the bag to take it off. The health orb moves when the lines land. Since E-3.5.

## Drops

Since E-2.32 a field monster can drop a gem when it dies: half the time at the host's default drop rate, a pickup at the corpse that anyone can walk over. Since E-3.8 the pickup shows the gem's own stone, the same icon as in the bag, inside a frame in its rarity's colour, hung a little above the ground and always the same size on screen whatever the camera distance, so a gold-framed ruby on the ground is told from a grey-framed jasper before anyone walks to it; a gem whose stone has no icon falls back to the old tinted sphere. The first player to touch it gets it in their bag; it vanishes for everyone, and an untaken one fades after two minutes. Each drop is one colour and one fixed line, from a list of five: a red one with flat damage (which does nothing yet), blue ones with maximum mana or mana regeneration, green ones with health or defense. With `Slime.AffixRoll` on (the default since E-3.3) a drop is rolled instead: a colour, a rarity by weight, always tier I for now, and its lines from the [affix table](affix-table.md); `Slime.AffixRoll 0` brings the five fixed lines back. What drops which tier waits on the real drop table (E-3.4). The host's Drop rates rule scales the chance.
...
Drops: `Equipment/GemDrops.*` (the roll), `Equipment/GemPickup.*` (the actor), `Equipment/GemDropSettings.*` and the `GemDropSettings` section of `DefaultGame.ini` (chance, lifetime, lines). Called from `Monsters/FieldMonster.cpp` `Die()`. The look (E-3.8): `AGemPickup` replicates `BaseId`, `Rarity` and `Tier` beside `Colour` (push model; the lines stay on the server), and `ApplyLook`, from BeginPlay and every OnRep, asks `UInventoryLayoutSettings::FindGemIcon` for the stone and, when there is one, hands it with `FindRarityColour(Rarity)` to `UGemPickupWidget` (`UI/GemPickupWidget.*`, a size box, a border and an image) on the pickup's screen-space `UWidgetComponent`, hiding the sphere; no icon, or a server with no viewport, keeps the sphere. Rarity colours are `GemRarities` rows (R1, R2, R3) and `RarityUnknownColour` in `UInventoryLayoutSettings`, with `PickupIconSize`, `PickupBorderThickness` and `PickupIconRise`; the bag cell (`UGemItemCell::SetGem`) and the slot (`UGemSlotWidget::Refresh`) paint the same colour on their frame, and `UGemItemCell::FormatAffixValue` formats a value per `EAffixOperation` for the tooltip, which adds the row's `Tier`, `MinValue` and `MaxValue` from `DT_Affixes`. UI placeholders, no marker; what rarities exist is D-3.3's.
