...
- **One move at a time, with a buffer.** A press during the commit window does nothing, except in its last fifth of a second: that press is kept, newest wins, and fires the frame the window ends, so mashing on the beat chains swings without a gap. A buff's animation never blocks: another key cuts it short and fires.
- **A buff's aura lasts exactly as long as the buff** (since E-2.98). Bone Ward's glow used to stay on forever and Festering Shroud's ended a second early, because the effect's length was typed on the soul asset separately from the buff's length in the move table, and the two had drifted (0 and 5 against 8 and 6 seconds). The aura now reads the move table's `BuffDuration`, so there is one number.
- **The same move waits for its own swing to finish** (since E-2.93). The commit window is short so that a *different* move or a step can follow a swing quickly; it used to let the *same* free attack fire again as well, so mashing one key landed a swing every half second while each animation was still playing (the 2026-09-21 playtest's attack spam). Now a move cannot be fired again until its own animation, at its play rate, would have ended: about 0.9 seconds on the basic attacks. Walking away mid-swing does not shorten the wait. A press in the last fifth of a second of that wait is buffered like any other. Moves with a cooldown longer than their animation are unchanged.
- **Swing speed is a number in a table.** Every move's animation plays at its row's play rate (1.5 on the basic attacks, so a 1.33 s swing takes 0.89 s); its contact frame moves with it. Attack speed as a stat is design's call (D-2.9); until then the rate is a balance placeholder.
- **Hits land with weight.** A landed hit freezes both bodies for a few hundredths of a second and gives the attacker's own camera a small shake. A monster hit mid-swing flinches (the freeze) without losing its swing.
- **You stand still for a move, briefly.** Since E-2.71 a move stops your monster where it is; since E-2.72 it holds you only through the swing's commit window (about a third of a second on the basics), then the rest of the animation is recovery: click and you walk out of it, press another move and it fires at once. A mouse button you keep holding moves you again the moment the window ends. Two exceptions: charges (Marrow Rush) carry you, and buffs (Festering Shroud, Bone Ward, any move with a buff duration) never stop you at all: their animation plays while you stand, and walking or pressing another move cuts it short while the buff itself stays on. `Slime.RootDuringMove 0` brings the whole-clip slide back; `Slime.AttackFeel 0` brings back the whole-clip commit.
...
E-2.98: in `USoulComponent::PlayMoveEffect` an `OnSelf` effect of a move whose `DT_MoveStats` row has a `BuffDuration` is deactivated after that time; `FMoveEffect::Duration` on the asset still rules every other effect. Read on each machine from the same table, not from the running GameplayEffect, so a buff dispelled early would keep its aura until its time (no such path exists yet).

E-2.93: `USoulComponent::ActionMoveClass` and `ActionRefireEndTime` are set when the authority accepts a move (`ActivateMove`) and on every machine that receives the move's multicast (`PlayMove`, clip played or not); they are separate from `ActionClipEndTime` because `UpdateLocomotion` zeroes that one the moment the pawn walks out of a recovery. `GetMoveRefireRemaining(Ability)` is 0 for any other move and with `Slime.AttackFeel 0`. The authority refuses with `pressed '<move>' with N s of its clip left; refused`; the owning client drops or buffers the press in `AMonsterCharacter::PressAbilityKey`, which now looks the slot's move up before the busy check. Field monsters were already paced by the whole clip (their attack task waits on `IsMoveInProgress`).
